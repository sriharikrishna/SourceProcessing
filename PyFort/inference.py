...
def _typemerge(lst,default,addLength=False):
   DebugManager.debug('inference._typemerge called on '+str(lst)+'...',newLine=False)
...
def _constantType(e,lineNumber):
...
def __identifierType(anId,localSymtab,lineNumber):
   ...
   # an entry exists with no type -> try to type implicitly
      ...
      DebugManager.warning('inference.__identifierType: implicit typing: '+symtabEntry.typePrint()+' '+anId,lineNumber,DebugManager.WarnType.implicit)
   else: # no symtab entry -> try local implicit typing
      returnType = localSymtab.implicit[anId[0]]
      if (returnType):
         DebugManager.warning('inference.__identifierType: implicit typing: '+SymtabEntry.ourTypePrint(returnType)+' '+anId,lineNumber,DebugManager.WarnType.implicit)
   if not returnType:
      raise InferenceError('inference.__identifierType: No type could be determined for identifier "'+anId+'"',lineNumber)
   return returnType
...
def __intrinsicType(anIntrinsicApp,localSymtab,lineNumber):
   ...
   #nonstandard ones: we check is_intrinsic before this is called.
      ...
      return _typemerge([expressionType(anArg,localSymtab,lineNumber) for anArg in anIntrinsicApp.args],
                       (None,None))

def __genericFunctionType(aFunctionApp,localSymtab,lineNumber):
   # find the symbol:
   ...
   raise InferenceError('inference.__genericFunctionType: could not resolve generic "'+aFunctionApp.head+'"',lineNumber)
...
def appType(anApp,localSymtab,lineNumber):
   ...
   # nonintrinsics: Look for it in the symbol table or for implicit type
      ...
      returnType = __identifierType(anApp.head,localSymtab,lineNumber)
      if (returnType is None) :
         # this must be a generic
         returnType=__genericFunctionType(anApp,localSymtab,lineNumber)
      DebugManager.debug(' It is an NONINTRINSIC of type '+str(returnType))
   return returnType

def __selectionType(aSelectionExpression,localSymtab,lineNumber):
   ...
   pType=__identifierType(dType[1][0]+":"+aSelectionExpression.proj,localSymtab,lineNumber)
   return pType
...
def expressionType(anExpression,localSymtab,lineNumber):
   ...
   if isinstance(anExpression,str) and is_const(anExpression):
      rType=_constantType(anExpression,lineNumber)
      ...
      return __identifierType(anExpression,localSymtab,lineNumber)
      ...
      if (anExpression.op =='//'):
         return _typemerge([expressionType(anExpression.a1,localSymtab,lineNumber),
      ...
      return __selectionType(anExpression,localSymtab,lineNumber)
...
def __shapecompare(d1,d2):
   DebugManager.debug('inference.__shapecompare called on d1 = "'+str(d1)+'\td2 = "'+str(d2)+'"')
   if d1 is None :
      return d2
   return d1

def __shapemerge(lst,default):
   DebugManager.debug('inference.__shapemerge called on '+str(lst)+'...',newLine=False)
   if not lst: return default
   if len(lst) == 1: return lst[0]
...
def __constantShape(e,lineNumber):
   ...
   if e[0] in _quote_set:
      return None
   raise InferenceError('inference.__constantShape: No shape could be determined for "'+e+'"',lineNumber)
...
def __arrayReferenceShape(arrRefApp,localSymtab,lineNumber):
   DebugManager.debug('inference.__arrayReferenceShape called on '+repr(arrRefApp)+'...',newLine=False)
   symtabEntry=None
